			...
			<tbody class="fc-2">
				<c:forEach items="${reserveList }" var="reserveVO">
					<tr>
						...
						<td scope="col">${reserveVO.num_people}様
						</th>
						<td scope="col">${reserveVO.check_in}~${reserveVO.check_out }
						</th>
						<td scope="col">
							<fmt:formatNumber value="120230" pattern="#,###.##" />
							円
						</th>
						<td scope="col">
							<c:choose>
								<c:when test="${reserveVO.re_status == 2}">
								予約完了
							</c:when>
								<c:otherwise>
								決済待ち
			...
			<tbody>
				...
				<tr>
					...
					<td class="fm" colspan="3">
						...
						<input type="text" class="ipt fw-5" title="メールドメイン" name="rsvEmail2" id="rsvEmail2" value="${loginMember.email }" maxlength="30" style="width: 144px; background: rgb(245, 245, 245);" disabled="disabled" readonly="readonly">
				...
				<tr>
					...
					<td class="fm" colspan="1">${loginMember.mber_grade }</td>
